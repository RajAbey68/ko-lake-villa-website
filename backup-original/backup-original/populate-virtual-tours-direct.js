/**
 * Ko Lake Villa - Direct Database Population for Virtual Tours
 * Using authentic iPhone photos from the property
 */

import { db } from './server/db.js';
import { virtualTours } from './shared/schema.js';
...
async function populateVirtualTours() {
  ...
  try {
    for (const tourData of virtualToursData) {
      const result = await db.insert(virtualTours).values(tourData).returning();
      console.log(`✓ Added virtual tour: ${tourData.roomName}`);
    }
    console.log('Virtual tours population completed successfully!');
  } catch (error) {
    console.error('Error populating virtual tours:', error);
  }
}
