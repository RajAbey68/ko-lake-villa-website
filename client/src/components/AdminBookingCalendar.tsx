...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from './ui/select';
import { Badge } from './ui/badge';
import { Calendar, DollarSign, Percent, Clock, Users } from 'lucide-react';
...
export default function AdminBookingCalendar() {
  ...
  const calculateDealPrice = (basePrice: number, checkInDate: string, deal: Deal) => {
    const daysFromNow = Math.ceil((new Date(checkInDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
    
    if (deal.type === 'early-bird' && daysFromNow >= deal.minDays) {
      return basePrice * (1 - deal.discount / 100);
    }
    
    if (deal.type === 'late-deal' && daysFromNow <= deal.minDays) {
      return basePrice * (1 - deal.discount / 100);
    }
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex justify-between">
                ...
                <Badge variant="outline">$12,890</Badge>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Deal Price Calculator</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="flex items-end">
              <Button className="w-full">Calculate Deals</Button>
            </div>
          </div>
        </CardContent>
      </Card>
