...
# Add a real SVG so we don't ship /images/hero.svg 404s
...
# 2) REPLACE /images/hero.svg REFERENCES
...
if [ -n "$rg_cmd" ]; then
  ...
  rg -l "placeholder\.svg" | while read -r f; do
    ...
    echo "• replaced /images/hero.svg in $f"
  done
else
  grep -RIl "/images/hero.svg" . --exclude-dir=node_modules --exclude-dir=.next --exclude-dir=.git 2>/dev/null | while read -r f; do
    sed -i.bak "s#placeholder\.svg#/images/hero.svg#g" "$f" || true
    echo "• replaced /images/hero.svg in $f"
  done
fi
...
echo " • /images/hero.svg replaced by /images/hero.svg"
